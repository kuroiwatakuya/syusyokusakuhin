...
void main(in PS_IN In, out float4 outDiffuse : SV_Target)
{
    ...
    //鏡面反射?
    float3 r = 2.0 * normal * dot(normal, 1) - 1;
    float3 v = normalize(Eyepos.xyz - In.WorldPosition.xyz);
    float i = pow(saturate(dot(r, v)), outDiffuse.w);
    outDiffuse.rgb += i * outDiffuse.xyz * Light.Diffuse.xyz + rim;
}